	...
	if (is_array($OB_handlers) && count($OB_handlers))
		foreach ($OB_handlers as $OB_last_handler)
			if (!in_array($OB_last_handler, $OB_default_handlers))
				echo GOTMLS_error_div(sprintf(__("Another Plugin or Theme is using '%s' to handle output buffers. <br />This prevents actively outputing the buffer on-the-fly and could severely degrade the performance of this (and many other) Plugins. <br />Consider disabling caching and compression plugins (at least during the scanning process).",'gotmls'), GOTMLS_htmlspecialchars($OB_last_handler)));
	GOTMLS_display_header();
		...
		if ($match = preg_split('/==\s*Upgrade Notice\s*==\s+/i', $ret)) {
			...
			if (preg_match_all('/=\s*([\.0-9]+)\s*=\s*([^=]+)/i', $notice[0], $matches, PREG_SET_ORDER)) {
				foreach ($matches as $m)
					$upgrade_notice .= GOTMLS_html_tags(array('li' => GOTMLS_html_tags(array('b' => esc_html($m[1]).':')).esc_html($m[2])));
				$upgrade_notice = GOTMLS_error_div($upgrade_notice, "GOTMLS_upgrade_notice");
				set_transient($transient_name, $upgrade_notice, DAY_IN_SECONDS);
			}
		}
	}
	echo wp_kses($upgrade_notice, array('div' => array('class' => array()),'li' => array(),'b' => array()));
}
